...
Majority_ConsensusORFs = collections.OrderedDict()
...
#GLIMMER_Input = open('./Input_Data/K-12/GLIMMER_K-12.txt', mode='rb')
...
#GeneMark_S_Input = open('./Input_Data/K-12/GeneMark_S_K-12.txt', mode='rb')
...
#GeneMark_HA_Input = open('./Input_Data/K-12/GeneMark_HA_K-12.txt', mode='rb')
...
#MetaGeneMark_Input = open('./Input_Data/K-12/MetaGeneMark_K-12.txt', mode='rb')
...
#MetaGeneAnnotator_Input = open('./Input_Data/K-12/MetaGeneAnnotator_K-12.txt', mode='rb')
...
#Prodigal_Input = open('./Input_Data/K-12/Prodigal_K-12.txt', mode='rb')
...
#FragGeneScan_Input = open('./Input_Data/K-12/FragGeneScan_K-12.txt', mode='rb')
...
Collection.update(PROKKA_ORFS)
Collection.update(Prodigal_ORFs)
Collection.update(GLIMMER_ORFs)
Collection.update(GeneMark_S_ORFs)
Collection.update(GeneMark_HA_ORFs)
Collection.update(MetaGeneAnnotator_ORFs)
Collection.update(MetaGeneMark_ORFs)
Collection.update(FragGeneScan_ORFs)


Seen_ORFs = collections.OrderedDict()
Seen_ORFs.update({0:1})

for key, po in Collection.items():   #All orfs?
    Count = 0
    Start = int(po.split(',')[0])
    Stop = int(po.split(',')[1])
    Di = str(po.split(',')[2])

    if Start in Seen_ORFs.keys() and Seen_ORFs[Start] != Stop:
        print "Seen"
    else:
        for ckey, cpo in Collection.items():
            cStart = int(cpo.split(',')[0])
            cStop = int(cpo.split(',')[1])
            cDi = str(cpo.split(',')[2])
            if Start == cStart and Stop == cStop and Di in cDi:
                Seen_ORFs.update({Start: Stop})
                Count +=1
            if Count >= 0:
                po = str(Start)+','+str(Stop)
                Majority_ConsensusORFs.update({po: Di})

print "Done"

print len(Majority_ConsensusORFs)

print Majority_ConsensusORFs


##############################
Majority = open('./Majority_Data/Myco_Majority.txt', mode='wb')

for po, Di in Majority_ConsensusORFs.items():
    Start = int(po.split(',')[0])
    Stop = int(po.split(',')[1])
    Majority.write(str(Start)+','+str(Stop)+','+str(Majority_ConsensusORFs[po])+'\n')
