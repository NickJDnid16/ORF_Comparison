...
#############################
    ...
    ORFs_A = np.zeros((Genome_Size), dtype=np.int)
    for O_Start, O_Stop in ORFs.items():
        ORFs_A[O_Start:O_Stop] = [1] * (O_Stop - O_Start)  # Changing all between the two positions to 1's
    ...
    ATG_Percentage = float((ATG) * float(100) / float(ORF_Num))
    GTG_Percentage = float((GTG) * float(100) / float(ORF_Num))
    TTG_Percentage = float((TTG) * float(100) / float(ORF_Num))
    ATT_Percentage = float((ATT) * float(100) / float(ORF_Num))
    CTG_Percentage = float((CTG) * float(100) / float(ORF_Num))
    ...
    NOT_O = np.logical_not(ORFs_A) + [0 for i in xrange(len(ORFs_A))]
    NOT_O_AND_Gene = NOT_O & Gene_A
    NOT_O_AND_Gene_Count = np.count_nonzero(NOT_O_AND_Gene)
    print NOT_O_AND_Gene_Count

    NOT_G = np.logical_not(Gene_A) + [0 for i in xrange(len(Gene_A))]
    NOT_G_AND_ORF = NOT_G & ORFs_A
    ...
    Output = [len(ORFs),ORF_Num,len(GCovered),Average_Length, Min_Length, Max_Length, Median_Start_Precision, Quarterly_Start_Precison, Median_Stop_Precision
              ...
              format(ATG_Percentage, '.2f'), format(GTG_Percentage, '.2f'), format(TTG_Percentage, '.2f'),format(ATT_Percentage, '.2f'),format(CTG_Percentage, '.2f'),format(Other_Percentage, '.2f'),
