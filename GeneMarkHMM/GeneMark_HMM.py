...
GeneMark_HMMInput = open('./GeneMark_HMM_Staph_K-12.txt', mode='rb')

GeneMark_HMMOutput = open('./GeneMark_HMM_Metrics.csv', mode='wb')
...
for line in GeneMark_HMMInput:
    if "Chromosome" in line :
        ...
        GeneMark_HMMORFs.update({Start: Stop})

Output, Start_Precision, Stop_Precision = orfComparison(Genes, GeneMark_HMMORFs, Start_Codons,Stop_Codons, Genome)

GeneMark_HMMOutput.write(str(Output)+'\n')
GeneMark_HMMOutput.write(str(Start_Precision)+'\n')
GeneMark_HMMOutput.write(str(Stop_Precision)+'\n')
