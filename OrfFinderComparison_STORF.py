...
def orfComparison(Genes,ORFs,Start_Codons,Genome):
    ...
    #####
    FP_ORFs = open('./FP_ORFs.txt', mode='wb')
    FP_ORFs_RC = open('./FP_ORFs_RC.txt', mode='wb')
    #####
...
#############################
    ...
    for O_po, O_St in ORFs.items():
        ...
        O_Stop = int(O_po.split(',')[1])
        try:
            print ORFsFiltered[O_Start]
            if O_Start not in ORFsFiltered.keys() and ORFsFiltered[O_Start] != O_Stop:
