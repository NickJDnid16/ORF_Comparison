...
def orfComparison(ORFs,Genes, Genome):
    ###################################################
    ...
    #####
   # FP_ORFs = open('./FP_ORFs_K-12.txt', mode='wb')
   # FP_ORFs_RC = open('./FP_ORFs_RC_K-12.txt', mode='wb')
    #####
    ...
    for positions in ORFs.itervalues():
        O_Start = int(positions.split(',')[0])
        O_Stop = int(positions.split(',')[1])
    ...
    Gene_AND = A & Gene_A
    print np.count_nonzero(Gene_A)
    print np.count_nonzero(Gene_AND)
    print 100 * float(np.count_nonzero(Gene_AND))/float(len(Gene_A))
